  ...
  return (
    <>
      <div className="h-full flex flex-col">
        ...
        <main className="flex-1 overflow-hidden">
          <Tabs defaultValue="editor" className="h-full flex flex-col">
            <TabsList className="w-full justify-start border-b rounded-none bg-muted/30 px-6">
              <TabsTrigger value="editor" className="data-[state=active]:bg-background data-[state=active]:shadow-sm">
                <FileText className="w-4 h-4 mr-2" />
                Minutes Editor
              </TabsTrigger>
              <TabsTrigger value="ai-tools" className="data-[state=active]:bg-background data-[state=active]:shadow-sm">
                <Brain className="w-4 h-4 mr-2" />
                AI Enhancement Tools
              </TabsTrigger>
              <TabsTrigger value="factcheck" className="data-[state=active]:bg-background data-[state=active]:shadow-sm">
                <CheckCircle className="w-4 h-4 mr-2" />
                Fact Verification
              </TabsTrigger>
            </TabsList>

            <TabsContent value="editor" className="flex-1 m-0 p-8">
              {minutes || transcript.length > 0 ? (
                <Textarea
                  value={minutes}
                  onChange={(e) => setMinutes(e.target.value)}
                  className="h-full resize-none text-base leading-relaxed focus-visible:ring-2 focus-visible:ring-primary/50 transition-all font-sans"
                  placeholder={`Begin typing your meeting minutes here...\n\n💡 Tips:\n• Click 'View Transcript' to reference the meeting conversation\n• Use 'Generate with AI' for an automated first draft\n• Press Ctrl+S (Cmd+S on Mac) to save your progress\n• The document auto-saves every few seconds`}
                />
              ) : (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <h2 className="text-2xl font-semibold mb-3 text-foreground">Ready to Create Minutes</h2>
                  <p className="text-muted-foreground max-w-md mb-6 text-base">
                    Start by generating AI-powered minutes or begin typing manually. Reference the transcript anytime by clicking "View Transcript".
                  </p>
                  <div className="flex gap-3">
                    <Button onClick={handleGenerateMinutes} disabled={transcript.length === 0} className="gap-2">
                      <Sparkles className="w-4 h-4" />
                      Generate with AI
                    </Button>
                    <Button variant="outline" onClick={() => setShowTranscriptModal(true)} className="gap-2">
                      <FileText className="w-4 h-4" />
                      View Transcript
                    </Button>
                  </div>
            ...
            <TabsContent value="ai-tools" className="flex-1 m-0 p-8 overflow-auto">
              <div className="max-w-5xl mx-auto">
                <div className="mb-6">
                  <h2 className="text-xl font-semibold mb-2">AI Enhancement Tools</h2>
                  <p className="text-muted-foreground">Use AI-powered tools to improve clarity, grammar, and structure of your minutes</p>
                </div>
                <AIMinutesEnhancer minutes={minutes} meetingId={meetingId!} onMinutesUpdate={setMinutes} />
              </div>
            </TabsContent>

            <TabsContent value="factcheck" className="flex-1 m-0 p-8">
              <div className="max-w-5xl mx-auto">
                <div className="mb-6">
                  <h2 className="text-xl font-semibold mb-2">Fact Verification</h2>
                  <p className="text-muted-foreground">Cross-reference minutes with meeting transcript for accuracy</p>
                </div>
        ...
        {/* Footer - Sensitive Sections */}
        <footer className="border-top bg-muted/5 p-6">
          <SensitiveSectionManager meetingId={meetingId!} sections={sensitiveSections} onUpdate={fetchMeetingData} />
        </footer>
      </div>
      
      <TranscriptMediaModal
